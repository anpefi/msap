...
options(keep.source = FALSE, width = 60)
...
\begin{description}
	...
	\item[enz1]{
		String for the label used in the datafile for the first enzyme. By default, as it considers standard MSAP with HPA/MSP, the value is "HPA". Note that this is case-sensitive and it should match the label given in the datafile.
	}
        \item[enz2]{
		String for the label used in the datafile for the second enzyme. By default, as it considers standard MSAP with HPA/MSP, the value is "MSP". Note that this is case-sensitive and it should match the label given in the datafile.
	}
	\item[threshold.poly.MSL]{
          	Minumum number of occurrences of each state (methylated/unmethylated) in a MSL to be defined as polymorphic. The default is 1, that gives the máximum number of polymorphic sites, as only on occurrence of any of the states is needed. 
	}
        \item[threshold.poly.NML]{
          	Minumum number of occurrences of each state (band/no band) in a NML to be defined as polymorphic. The default is 1, that gives the máximum number of polymorphic sites, as only on occurrence of any of the states is needed. 
        }
        \item[no.bands]{
  		This is a deprecated argument kept for compatilibility porpouses with previous versions. Use pattern.
	}
	\item[uninformative]{
		This is a deprecated argument kept for compatilibility porpouses with previous versions. Use pattern.
	}
